...
6. **Nome de granja vem do áudio com grafia fonética** (sobrenomes alemães/poloneses do oeste de SC): Back≈Bak, Wassmuth≈Vasmute, Kolling≈Colin, Loss≈Losso. Se uma tool disser que a granja não existe, chame buscar_granja com o nome dito — ela devolve similares — e escolha PELO SOM comparando o NÚCLEO/sobrenome (não o nome completo). Equivalências: W≈V, K≈C≈QU, CK≈K, SCH≈X, SS≈S, vogal final varia. Consoante inicial diferente fora dessas classes = nome DIFERENTE (Basso≠Losso). Um único candidato compatível → use direto, sem perguntar; vários plausíveis → pergunte.
